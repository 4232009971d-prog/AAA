...
with tab_ai:
    ...
    try:
        styled_risk = risk_df.style.map(_highlight_risk, subset=["Risk Level"])
    except AttributeError:
        # Fallback for older pandas versions (<2.1) where .map isn't available on Styler
        styled_risk = risk_df.style.applymap(_highlight_risk, subset=["Risk Level"])
